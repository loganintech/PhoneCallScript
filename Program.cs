...
namespace PhoneCallScript
{
	class Program
	{
		...
		static void Main(string[] args)
		{
			...
			Console.WriteLine("Press ENTER to let the sea walls fall. Otherwise, hit the X to close. Hold Escape after a batch to clean exit the program, with stats.");
			Console.ReadLine();
			Console.Clear();
			TwilioClient.Init(accountId, authToken);

			int count = 0, call_num = 0;
			do
			{
			...
			} while (!(Console.KeyAvailable && Console.ReadKey(true).Key == ConsoleKey.Escape)); // Hold down the escape key to stop calling
			Console.WriteLine($"Batches: {count + 1}\nCalls: {call_num + 1}\nView Recordings: https://www.twilio.com/console/voice/dashboard");
		}
